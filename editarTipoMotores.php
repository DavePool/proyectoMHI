...
<head> 
    ...
    <title>Edicion de Motores</title>
    <!-- Bootstrap core CSS -->
   ...
   <body>
	...
	if(isset($_GET['editar'])){
			...
			$consulta = "SELECT * FROM tipo_motores WHERE id_tipomotor='$editar_id'";
			$ejecutar = sqlsrv_query($con, $consulta);

			$fila = sqlsrv_fetch_array($ejecutar);

			$idP = $fila['id_tipomotor'];
...
<div class="col-md-8 col-md-offset-2">
		<form method="POST" action="">
			...
			<div class="form-group">
				<label>Descripcion Del Motor:</label>
...
<?php
	...
	if(isset($_POST['actualizar'])){
			...
			$consulta = "UPDATE Tipo_Motores SET id_tipomotor ='$actualizar_idProveedor', descripcion_motor='$actualizar_rfc' WHERE id_tipomotor ='$editar_id'";

			$ejecutar = sqlsrv_query($con, $consulta);

			if($ejecutar){
				echo "<script>alert('Datos actualizados')</script>";
				echo "<script>window.open('tablaTipoMotores.php', '_self')</script>";
			}else{
